...
| **011** | [The backlog: who files an item, and how one is identified](IDM-011-the-backlog.md) | 2026-09-04 | **A session asks before filing an item; it lands on the owner's word**, and a declined one is recorded in the phase's `notes.md` — which amends `IDM-001`'s authority claim while leaving its branch rule alone. Also the `BKL-NNNN` identifier allocated in **file order**, the metadata line that sits under each item's `### BKL-NNNN — title` heading, **one shape and only one**, five statuses of which only `done` leaves the file, and a **generated** table — the fourth time a hand-maintained count going stale has decided something here. Says outright what its own checker **cannot** see: the frozen archive's title citations |
