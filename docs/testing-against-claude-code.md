...
**There is no observability yet.** Phase 2 is still a stub, so nothing in this repository tells you
which backend a request took. The available signals are uvicorn's access log in the router terminal,
which shows requests arriving but not where they went, and **LM Studio's own server log**, which is
the reliable way to confirm a request reached the local model. Once Phase 2 lands, `logs/calls.csv`
answers this directly and most of this paragraph can go.
