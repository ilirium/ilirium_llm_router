...
**`IDM-009`, `IDM-010` and `IDM-011` are in force.** Reviewing **executed** work, which draws the
line between all three review protocols; the per-phase `for-the-owner.md`, written *during* a phase
and to a person; and the backlog — **who may file an item, and how one is identified.** One of
`IDM-011`'s rules was in force before the document landed: **a session asks the owner before filing
a backlog item**, which amends `IDM-001`'s authority claim while leaving its branch rule alone.

**Every backlog item now carries a permanent `BKL-NNNN` id.** 38 items across two files —
`../backlog.md` holds 34 and the new `../backlog-done.md` holds the four that are done — with a
table in each generated by `../procedures/backlog-index.py` from the items themselves.

***The count is 38 because a cold review found two items the compile pass had missed.*** It proposed
36; `backlog.md`'s two struck-through done entries open `~~**` where the pass enumerated openings as
bold-or-`###` at the line start, **so two bytes in the wrong order made two items invisible.** They
survived a compile, an author review and two forward-review passes. **Ids are permanent, so a missed
item is a wrong id frozen forever** — which is the whole reason the review ran before anything was
applied.

**The item shape changed after all of it shipped.** The owner read the finished files and rejected
it, so a late group converted every item to a `### BKL-NNNN — title` heading and **retired the two
shapes that had hidden the missing items**. No id moved; that was proved mechanically rather than
asserted.

**Phase 13 is `IDM-009`'s first subject, and the protocol stopped its own author's merge.** Two
runs, **23 findings, five method-tier or settled-row** — and **none of the 23 was caught by any
instrument the phase built**, with every check green. *Two of the findings are about instruments
that pass while testing nothing: a count assertion that cannot detect the failure it was built for,
and a table column named by two documents and written by no code.* **A third, of the same kind, was
found in the phase's own register check during Group G.**

*What it costs to know that is the phase's most transferable number: **13% overlap between the two
