...
| **004** | [Documentation structure at the milestone boundary](EPD-004-documentation-structure.md) | 2026-08-15 | proposal | a decision | The first EPD about the repository rather than the router. Milestone 1 is done and its documents no longer separate what is true from how it was found out. Proposes four tiers — durable reference, re-runnable procedures, EPDs, and a per-milestone archive — and a numbering rule: number what needs a stable identity to be cited, not what needs an order. The migration is planned separately in `docs/docs-restructure-plan.md` |
