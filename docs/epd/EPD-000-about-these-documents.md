...
| **003** | [Capturing bodies for a corpus](EPD-003-capturing-bodies-for-a-corpus.md) | 2026-07-31 | **decided** 2026-08-17 | — | Storing every request and response body for later analysis, and possibly as a fine-tuning corpus. Establishes that the corpus is ~93% duplicated prefix, that the storage question is a compression-window question rather than a database question, and that the fine-tuning half of the goal runs into Anthropic's terms. **Both load-bearing questions answered: fine-tuning dropped (analysis only), and per-call files are the unit** — its gate ran on 73 real bodies and a held-out dictionary reached 12.10× against 3.12× unaided. Graduates into `../reference/design-decisions.md`. Open questions 3–6 survive as Phase 10 design detail rather than as forks |
