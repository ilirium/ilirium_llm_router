...
| **003** | [Capturing bodies for a corpus](EPD-003-capturing-bodies-for-a-corpus.md) | 2026-07-31 | **partly accepted** | its gate — Phase 9 | Storing every request and response body for later analysis, and possibly as a fine-tuning corpus. Establishes that the corpus is ~93% duplicated prefix, that the storage question is a compression-window question rather than a database question, and that the fine-tuning half of the goal runs into Anthropic's terms. **The accepted piece: fine-tuning is dropped and the corpus is for analysis only, decided 2026-08-17** — its open question 1, which everything else was downstream of. What stays open is per-call files against per-session streams, which waits on the gate `../milestone-2-corpus/phase-9-corpus-gate/` runs |
